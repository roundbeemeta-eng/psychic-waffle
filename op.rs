// ==========================================================================
// PHANTOM SOCIAL v3.7 - STABILIZED EDITION ðŸ›¡ï¸?
// ==========================================================================
// âœ? FIXED: Stale Registry Flag causing immediate exit
// âœ? FIXED: COM Initialization in threads
// âœ? FIXED: Persistence thread race condition
// ==========================================================================
...
fn show_smart_notification(is_escalated: bool) {
    ...
    // FIXED: Safe unwrapping to prevent thread panics
...
fn notification_loop() {
    // FIXED: Initialize COM for this thread so Toast Notifications work
    unsafe {
        CoInitializeEx(ptr::null_mut(), COINIT_APARTMENTTHREADED);
    }
    ...
    loop {
        ...
        for _ in 0..20 {
            ...
            if check_activation_flag() {
                ...
                // Flag is cleared in main thread or here to be safe
...
fn main() {
    ...
    // FIXED: FORCE CLEAR FLAG TO PREVENT PREMATURE EXIT (ZOMBIE STATE)
    clear_activation_flag();
    ...
    loop {
        // Sleep to ensure no race condition on startup
