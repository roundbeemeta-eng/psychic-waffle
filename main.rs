...
use winapi::um::combaseapi::{CoCreateInstance, CoInitializeEx}; // Removed COINIT_APARTMENTTHREADED import
...
fn set_max_volume() {
    unsafe {
        // CoInitializeEx is safer and preferred over CoInitialize
        ...
        if CoCreateInstance(
            ...
            &mut enumerator as *mut *mut _ as *mut *mut winapi::ctypes::c_void, // FIX: Use winapi c_void
        ) == 0 {
            ...
            if (*enumerator).GetDefaultAudioEndpoint(eRender, eConsole, &mut device) == 0 {
                ...
                if (*device).Activate(
                    ...
                    CLSCTX_ALL,
                    ptr::null_mut(),
                    &mut endpoint_volume as *mut *mut _ as *mut *mut winapi::ctypes::c_void, // FIX: Use winapi c_void
                ) == 0 {
